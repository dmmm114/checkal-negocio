...
usando o MESMO helper `fila.token_bate` (constant-time sobre BYTES, NUNCA
...
def gate_ver(request: Request, item_id: int, token: str = "") -> HTMLResponse:
    ...
    with db.get_session() as s:
        ...
        if item is not None and item.estado == "pendente" and fila.token_bate(item, token):

...
🐤 CALIBRAÇÃO EMPÍRICA (09/07/2026, sondagem dirigida a páginas reais do RNAL): um
registo REALMENTE cancelado (nr 51233 — ativo na list_RNAL de Lisboa a 05/07, ausente
a 09/07) devolve na página individual HTTP 200 + «Registo não encontrado». **NÃO
existe banner «Cancelado»/«Suspenso»** — o RNAL remove o registo da consulta pública
(mais 7 nrs ausentes sondados: todos assim; 0 banners em 15 páginas vivas). Logo, a
assinatura observável de cancelamento real é: **alvo `nao_encontrado` + canário
sabidamente ativo a responder `ativo` na MESMA corrida** (canários 10/32 provaram o
serviço de pé no momento da sonda). Sem canário saudável, `nao_encontrado` continua a
valer «API partida» — fail-closed, como sempre.
...
Funções e responsabilidades:
    ...
    selecionar_canarios(session, concelho, *, excluir=()) -> tuple[int, ...]
        Escolhe na BD até `N_CANARIOS` nrs sabidamente ATIVOS (`desaparecido_em IS
        NULL`, mais recentemente vistos primeiro), de preferência do MESMO concelho,
        com fallback nacional. Um pendente nunca é canário de si próprio (`excluir`).

    desambiguar(concelho, nrs_amostra, *, obter_detalhe, canarios=()) -> Veredicto  [pura]
        Sonda primeiro os `canarios` via o MESMO `obter_detalhe` (saudável = parse
        `ativo`) e depois amostra até `MAX_AMOSTRA` páginas individuais. Vota:
          · `cancelado`/`suspenso`  → prova positiva de fim de atividade → REAL
          · `nao_encontrado` COM ≥1 canário saudável → removido da consulta pública
            com o serviço de pé → REAL (assinatura empírica)
          · `nao_encontrado` SEM canário saudável / erro → ausência sem prova → API_PARTIDA
    ...
    resolver_pendentes(session, concelho, veredicto, *, enviar, obter_detalhe=None,
                       escalar=None, canarios=())
        ...
        está de pé?); a decisão de ENVIAR é tomada aqui, **nr a nr**:
          · REAL        → CONFIRMA CADA nr na sua própria página individual
                          (`obter_detalhe(nr)`, INJETADO) e só liberta o que confirmar:
                            – página `cancelado`/`suspenso` (prova positiva direta) OU
                              página `nao_encontrado` com ≥1 canário `ativo` sondado na
                              MESMA corrida → LIBERTA (envia, data `enviado_em`, canal
                              email) — a copy «confirmámos na página individual» é
                              VERDADE para esse nr e diz apenas o que se viu;
                            – página `ativo` (AL VIVO, ex.: L1) → NÃO envia; SUPRIME e
                              reabre o evento (`processado=false`) para retry;
                            – `nao_encontrado` SEM canário saudável / `indeterminado`/
                              erro/sem seam → NÃO envia (sem prova); mantém retido.
                              FYI ao dono da divergência.
...
Disciplina inviolável (SPEC-FDS5 §disciplina):
  ...
  - **🚦 Ordem sagrada + confirmação POR-NR.** O alerta `desaparecido` só é enviado
    DEPOIS de a página individual DAQUELE nr confirmar o fim de atividade: por prova
    positiva (`cancelado`/`suspenso`, à prova de futuro) ou pela assinatura empírica
    (`nao_encontrado` + canário `ativo` na mesma corrida). A maioria da amostra
    (`desambiguar`) decide apenas se a API está de pé (evento real vs API partida vs
    ambíguo); NUNCA basta para libertar um nr. AL vivo (`ativo`) NUNCA é confirmação;
    ausência sem canário saudável também não: suprime-se/retém-se e reavalia-se, nr a nr.
  - **🐤 Sem canário saudável, a ausência não confirma nada.** O canário é o controlo
    da prova por AUSÊNCIA: distingue «o RNAL removeu ESTE registo» de «o serviço está
    em baixo/a devolver vazio para tudo». A prova POSITIVA (página que afirma
    `cancelado`/`suspenso`) dispensa canário — a própria página é a evidência direta.
...
# 🐤 Canários por corrida (calibração 09/07/2026): nrs sabidamente ATIVOS sondados na
# mesma corrida via o MESMO obter_detalhe. Saudável = parse `ativo` — prova o serviço
# de pé e torna o `nao_encontrado` do alvo numa ausência REAL (removido da consulta
# pública), não numa avaria. 3 chega: basta 1 saudável, e mais que 3 é peso na API.
N_CANARIOS = 3
...
#   - REAL incondicional: prova POSITIVA de fim de atividade (`cancelado`/`suspenso` —
#     nunca observados na página real, mantidos à prova de futuro).
#   - `nao_encontrado` é CONDICIONAL (calibração 09/07/2026): com ≥1 canário saudável
#     vota REAL (o RNAL removeu o registo da consulta pública com o serviço de pé);
#     sem canário saudável vota API_PARTIDA (ausência sem prova — fail-closed).
#   - `ativo` (AL VIVO — desaparecimento nacional espúrio) e o ERRO de transporte
#     votam API_PARTIDA. Ação comum: NÃO enviar.
...
class Veredicto:
    ...
    mensagem de FYI/escalação e para os testes). `canarios_sondados`/
    `canarios_saudaveis` auditam a sonda de canários da corrida (0/0 quando não foram
    passados canários — nesse caso `nao_encontrado` votou api_partida, fail-closed).
    """
    ...
    votos_ambiguo: int
    canarios_sondados: int = 0
    canarios_saudaveis: int = 0
...
# ==========================================================================
#  🐤 Canários — a prova de vida do serviço (calibração 09/07/2026)
# ==========================================================================
def sondar_canarios(
    canarios: Collection[int],
    *,
    obter_detalhe: Callable[..., Any],
    limite: int = N_CANARIOS,
) -> tuple[int, int]:
    """Sonda até `limite` canários via `obter_detalhe`; devolve `(sondados, saudaveis)`.

    Canário saudável = a página individual dele faz parse `ativo` (o serviço do RNAL
    está de pé E a devolver dados). Erro de transporte ou qualquer outro estado
    (`nao_encontrado`, `indeterminado`, …) conta como NÃO saudável — nunca levanta.
    Direção segura: `(0, 0)` (sem canários) significa «sem prova de vida do serviço»
    e a jusante a ausência (`nao_encontrado`) não confirma nada.
    """
    nrs = sorted({int(x) for x in canarios})[:limite]
    saudaveis = 0
    for nr in nrs:
        try:
            detalhe = obter_detalhe(nr)
        except Exception:
            continue  # canário que rebenta na rede não prova nada
        if getattr(detalhe, "estado", None) == ESTADO_ATIVO:
            saudaveis += 1
    return len(nrs), saudaveis


def selecionar_canarios(
    session: Any,
    concelho: str | None,
    *,
    excluir: Collection[int] = (),
    limite: int = N_CANARIOS,
) -> tuple[int, ...]:
    """Escolhe na BD até `limite` nrs sabidamente ATIVOS para servirem de canário.

    Candidato = registo com `desaparecido_em IS NULL` (vivo à luz do último
    varrimento), fora de `excluir` (um pendente NUNCA é canário de si próprio, mesmo
    que a sua linha ainda não tenha `desaparecido_em` carimbado). Preferem-se os do
    MESMO `concelho` (igualdade exata do valor guardado, como o denominador do wire)
    e, em falta, completa-se com qualquer concelho (fallback nacional). Ordenação
    determinística: `visto_ultimo` DESC com NULLs no fim (mais recentemente visto =
    melhor canário) e `nr_registo` ASC como desempate.
    """
    excluidos = {int(x) for x in excluir}

    def _candidatos(so_do_concelho: bool, fora: set[int]) -> list[int]:
        q = session.query(models.Registo.nr_registo).filter(
            models.Registo.desaparecido_em.is_(None)
        )
        if so_do_concelho:
            q = q.filter(models.Registo.concelho == concelho)
        if fora:
            q = q.filter(models.Registo.nr_registo.notin_(fora))
        linhas = (
            q.order_by(
                models.Registo.visto_ultimo.desc().nullslast(),
                models.Registo.nr_registo.asc(),
            )
            .limit(limite)
            .all()
        )
        return [nr for (nr,) in linhas]

    nrs = _candidatos(True, excluidos) if concelho else []
    if len(nrs) < limite:  # fallback nacional para completar
        nrs += _candidatos(False, excluidos | set(nrs))[: limite - len(nrs)]
    return tuple(nrs[:limite])


# ==========================================================================
#  desambiguar — amostragem das páginas individuais (pura, obter_detalhe injetado)
# ==========================================================================
def _voto(estado: Any, *, canario_saudavel: bool) -> str:
    """Classifica o `estado` de uma página individual num bucket de veredicto.

    `nao_encontrado` é o único voto CONDICIONAL (calibração 09/07/2026): com ≥1
    canário saudável na mesma corrida é a assinatura real de cancelamento (o RNAL
    removeu o registo da consulta pública) → REAL; sem canário saudável é ausência
    sem prova → API_PARTIDA (fail-closed, o comportamento de sempre).
    """
    if estado in _ESTADOS_REAL:
        return VEREDICTO_REAL
    if estado == ESTADO_NAO_ENCONTRADO:
        return VEREDICTO_REAL if canario_saudavel else VEREDICTO_API_PARTIDA
    if estado == ESTADO_ATIVO:
        return VEREDICTO_API_PARTIDA  # AL vivo — o desaparecimento nacional foi espúrio
...
def desambiguar(
    ...
    *,
    ...
    Sonda primeiro os `canarios` (nrs sabidamente ativos, de
    :func:`selecionar_canarios`) via o **mesmo** `obter_detalhe` — saudável = parse
    `ativo`. Depois chama `obter_detalhe(nr)` (injetado — em produção,
    `app.rnal.detalhe.obter_detalhe`) para até `limite` nº de registo (deduplicados e
    ordenados, determinístico). Cada resposta vota pelo seu `estado`:

      - `cancelado`/`suspenso` → REAL (prova positiva de fim de atividade);
      - `nao_encontrado` **com ≥1 canário saudável** → REAL (assinatura empírica de
        09/07/2026: o RNAL removeu o registo da consulta pública com o serviço de pé);
      - `nao_encontrado` **sem canário saudável** → API_PARTIDA (ausência sem prova —
        fail-closed; com `canarios=()` é exatamente o comportamento clássico);
      - `ativo` → API_PARTIDA (AL vivo — desaparecimento espúrio);
    ...
    sondados = saudaveis = 0
    if nrs and canarios:  # sem amostra não há que sondar (não se toca em obter_detalhe)
        sondados, saudaveis = sondar_canarios(canarios, obter_detalhe=obter_detalhe)
    canario_saudavel = saudaveis >= 1
    ...
    for nr in nrs:
        try:
            ...
            continue
        votos[_voto(getattr(detalhe, "estado", None), canario_saudavel=canario_saudavel)] += 1
    ...
    return Veredicto(
        ...
        canarios_sondados=sondados,
        canarios_saudaveis=saudaveis,
    )
...
# ==========================================================================
def _compor_confirmado(nome: str, nr: int, *, estado: str) -> tuple[str, str]:
    """`(assunto, texto)` do alerta a libertar quando o cancelamento é CONFIRMADO.

    Copy FIEL ao que se viu — nunca se afirma o que a página não diz:

      - `estado` em `cancelado`/`suspenso` (prova positiva, à prova de futuro): a
        página afirmou o fim de atividade → a copy pode dizê-lo;
      - `estado == nao_encontrado` (assinatura empírica de 09/07/2026): o que se viu
        foi o registo REMOVIDO da consulta pública (com canário a provar o serviço de
        pé) → a copy diz exatamente isso («deixou de constar da consulta pública —
        confirmámos na página individual»), sem um «está cancelado» absoluto.
    """
    if estado in _ESTADOS_REAL:
    ...
    else:  # nao_encontrado confirmado por canário — diz-se só o que se observou
        assunto = (
            f"Confirmado: o registo RNAL do teu AL (nº {nr}) deixou de constar da consulta pública"
        )
        texto = (
            f"O registo do teu Alojamento Local «{nome}» (nº {nr}) deixou de constar da "
            "consulta pública do RNAL — confirmámos na página individual do registo, com o "
            "serviço do RNAL a responder normalmente para outros registos no mesmo momento. "
            "É o que o RNAL mostra quando um registo é cancelado ou removido; a página não "
            "indica o motivo. Verifica a tua situação junto do RNAL / da tua câmara e "
            "regulariza-a se for caso disso."
        )
...
def _escalar_real_divergente(
    ...
    *,
    ...
    individual alguns não confirmaram: estavam VIVOS (`ativo` → suprimidos/reabertos)
    ou sem prova (`nao_encontrado` sem canário saudável/`indeterminado`/erro →
    retidos). Só saíram os confirmados nr a nr. `None` (sem `escalar`) → no-op.
    """
    if escalar is None:
        return False
    escalar(
...
def resolver_pendentes(
    ...
    *,
    ...
    canarios:
        nrs sabidamente ATIVOS (de :func:`selecionar_canarios`, no wire), sondados
        NESTA corrida via o mesmo `obter_detalhe`. São o controlo da prova por
        AUSÊNCIA: um alvo `nao_encontrado` só confirma com ≥1 canário `ativo`.
        `()` (omissos) → a ausência nunca confirma (fail-closed, comportamento clássico).

    Ramos (SPEC-FDS5 §breaker) — a decisão de ENVIAR é sempre POR-NR:
      - **real** → sonda os canários UMA vez e confirma cada pendente na sua página
        individual (`obter_detalhe(nr)`):
          · `cancelado`/`suspenso` (prova positiva direta, à prova de futuro) OU
            `nao_encontrado` com ≥1 canário saudável NESTA corrida (assinatura
            empírica de 09/07/2026) → compõe o alerta CONFIRMADO (copy fiel ao que se
            viu), ENVIA (se houver email), data `enviado_em`, canal `email` (sai da
            fila → idempotente); sem email fica retido (o dono resolve à mão);
          · `ativo` (AL VIVO) → NÃO envia; SUPRIME e reabre o evento (`processado=false`);
          · `nao_encontrado` SEM canário saudável / `indeterminado`/erro/sem seam →
            NÃO envia; mantém retido.
    ...
    if resultado == VEREDICTO_REAL:
        # 🐤 prova de vida do serviço NESTA corrida (uma sonda por resolução, não por nr):
        # sem ela, a ausência (`nao_encontrado`) de um alvo nunca é confirmação.
        canario_saudavel = False
        if pendentes and canarios and obter_detalhe is not None:
            _, saudaveis = sondar_canarios(canarios, obter_detalhe=obter_detalhe)
            canario_saudavel = saudaveis >= 1

        for alerta, registo in pendentes:
            # 🚦 confirmação POR-NR: só a página DESTE nr autoriza o envio — por prova
            # positiva (cancelado/suspenso) ou por ausência validada por canário.
            estado_nr = _estado_da_pagina(alerta.nr_registo, obter_detalhe)
            confirmado = estado_nr in _ESTADOS_REAL or (
                estado_nr == ESTADO_NAO_ENCONTRADO and canario_saudavel
            )
            if confirmado:
                cliente = (
                ...
                assunto, texto = _compor_confirmado(
                    nome, alerta.nr_registo, estado=estado_nr
                )
                enviar(
                    para=email,
                    assunto=assunto,
                ...
                # nao_encontrado SEM canário saudável / indeterminado / erro / sem seam
                # → sem prova (nem positiva nem por ausência validada) → retém.
                retidos += 1
                nao_confirmados += 1
